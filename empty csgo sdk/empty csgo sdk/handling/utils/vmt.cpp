...
namespace handling {
	namespace utils {
		...
		uintptr_t* vmt_hook::search_free_data_page(const char* module_name, const std::size_t vmt_size) {
		...
		vmt_hook::vmt_hook() : class_base(nullptr), vftbl_len(0), new_vftb1(nullptr), old_vftbl(nullptr) {
		}

		vmt_hook::vmt_hook(void* base) : class_base(base), vftbl_len(0), new_vftb1(nullptr), old_vftbl(nullptr) {
		}
		...
		bool vmt_hook::Initialize(void* base, const char* module_name) {
			if (base != nullptr)
				class_base = base;
		...
		void* vmt_hook::GetOriginalFunction(size_t fn_index) {
